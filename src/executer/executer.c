...
{
	if(ft_strncmp(command, "echo", ft_strlen("export")) == 0)
		ft_echo();
...
/*
void execute_single_bin(t_shell *shell, t_table_cmd *table)
{
	char	*path;
	char	*command;
	pid_t	pid;
	...
	execve(table->command, table->args, shell->ownenvp);
}
*/

int	command_redirection(t_table_cmd *table)
{
	t_list *redir;
...
//	shell->fd_out = dup(STDOUT_FILENO);
	...
	if(num_commands == 1)
	{
		temp_cmd_table = (t_table_cmd*)shell->cmd_list->content;
	//	if(isbuiltin(temp->command))
			execute_builtin(shell, temp->command);
	//	else
